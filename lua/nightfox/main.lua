...
function M.load(opts)
  ...
  -- User generated precompiled file with `NightfoxCompile`
  if util.exists(precompiled_file) then
    cmd("luafile " .. precompiled_file)
    return
  end

  -- Try to load the default precompiled file if:
  -- - No store entry
  -- - No config set
  -- - debug value is not set
  if store.is_empty(name) and not config.has_options and not vim.g.nightfox_debug then
